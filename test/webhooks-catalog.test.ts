...
describe('client.webhooks y client.catalog', () => {
  ...
  describe('endpoints', () => {
    ...
    it('evento de prueba entregado', async () => {
      ...
      // El contrato exige `error` y lo declara `string | null`: en una entrega
      // correcta viaja como null, no ausente.
      assert.equal(result.attributes.error, null);
    });
    ...
    it('rotar el secreto devuelve el nuevo', async () => {
      ...
      assert.equal(
        endpoint.attributes.secret,
        'ffeeddccbbaa99887766554433221100f9e8d7c6b5a4938271605f4e3d2c1b0a',
      );
