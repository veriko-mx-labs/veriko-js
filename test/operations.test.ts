 ...
 * o es pública con `security: []`. Una operación que no cumpla eso no puede
 * entrar ni en este spec ni en el SDK.
 *
 ...
 * pública y no pide autenticación.
 */
...
/** La superficie M2M completa deriva del spec público. */
  ...
  for (const testCase of CASES) {
    ...
    it(label, async () => {
      ...
      try {
        ...
        assert.ok(acceptsApiKey(operation), `${template} no acepta la clave de API`);
...
describe('el conjunto de operaciones M2M cubierto', () => {
  ...
  it('toda operación del spec acepta la clave de API', () => {
    const withoutApiKey = allOperations()
    ...
    assert.deepEqual(
      withoutApiKey,
      [],
      `Operaciones que no aceptan la clave de API: ${withoutApiKey.join(', ')}`,
    );
    assert.deepEqual(
      withoutApiKey.filter((operationId) => implemented.has(operationId)),
      [],
    );
  });

  it('exige que la lista de excepciones esté vacía antes de liberar', () => {
...
it('los planes públicos funcionan sin clave de API y no envían Authorization', async () => {
  const server = await RecordingServer.start();
  try {
