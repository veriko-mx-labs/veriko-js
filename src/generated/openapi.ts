...
export interface paths {
    ...
    "/users/me/sessions": {
        ...
        /**
         * Listar sesiones web (JWT) activas de una cuenta de usuario
         * @description Devuelve las sesiones web con token JWT activas del usuario autenticado.
         *
         *     El orden de lista es por "Uso reciente" y las sesiones revocadas/caducadas
         *     no se muestran.
         *
         *     La sesión que hace la llamada se reconoce por `is_current=true`, así que
         *     destacarla no cuesta otra petición.
         */
        get: operations["listMySessions"];
        put?: never;
        post?: never;
        /**
         * Revocar todas las sesiones web (JWT) de una cuenta de usuario
         * @description Revoca varias sesiones del usuario autenticado en una sola llamada. Lo que
         *     hace exactamente lo decide el parámetro `except`:
         *
         *     - Sin `except`: revoca **todas** (incluida la actual), revoca las
         *       credenciales del inicio de sesión biométrico y deja sin servicio la clave
         *       de API vigente, que hay que regenerar. La respuesta retira las cookies de
         *       autenticación, por lo que la petición siguiente ya no va autenticada.
         *     - Con `except=current`: revoca todas **menos** la que hace la llamada. Ni
         *       las credenciales de dispositivo ni la clave de API se tocan.
         *
         *     La respuesta dice en `mode` cuál de los dos caminos se tomó y en
         *     `revoked_count` cuántas sesiones se cerraron. Cualquier otro valor de
         *     `except` provoca un estado HTTP `422` (con `invalid_except_value` en el
         *     cuerpo).
         *
         *     {% callout type="info" %}
         *     El camino sin `except` es el de «alguien entró en mi cuenta»: corta todos
         *     los accesos anteriores, incluido el biométrico rápido, que de otro modo
         *     volvería a abrir sesión de inmediato.
         *
         *     {% /callout %}
         */
        delete: operations["revokeMySessions"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    ...
    "/users/me/sessions/{id}": {
        ...
        post?: never;
        /**
         * Revocar una sesión web (JWT) de una cuenta de usuario
         * @description Revoca la sesión web asociada al `id` indicado en la petición. Los
         *     identificadores pueden ser obtenidos desde `GET /v1/users/me/sessions`.
         *
         *     La consulta que la marca va acotada a la cuenta de usuario que llama, así
         *     que revocar la sesión de otra es imposible aunque se conozca su `id`.
         *
         *     Un `id` desconocido, uno ajeno y uno ya revocado comparten respuesta —un
         *     estado HTTP `404` (con `session_not_found_or_already_revoked` en el
         *     cuerpo)—, de modo que nunca se delata qué identificadores existen.
         *
         *     Un `id` que no sea un UUID responde con un estado HTTP `422` (con
         *     `invalid_session_id` en el cuerpo).
         */
    ...
    "/beneficiaries/validate-account": {
         ...
         * @description Comprueba la estructura de un número de cuenta. El tipo de cuenta se detecta por la longitud: CLABE (18 dígitos), tarjeta (16 dígitos), celular/DiMo (10 dígitos).
         *
         *     Se verifica el **dígito de control de la CLABE** o el **Luhn de la tarjeta**, y se resuelve el banco.
         *
         *     Responde con un estado HTTP `200` siempre que llegue `account`. Un número mal formado no es un error de la petición, sino un resultado: se refleja en `checksum_valid=false` o en `account_type='unknown'`, no con un estado HTTP `422`.
         *
         *     No consume {% concept slug="quotas-and-plans" %}cuota del plan{% /concept %}.
         */
    ...
    "/validations/imports": {
         ...
         * @description Crea un trabajo de importación para validar varias transferencias y obtener el CEP de cada una.
         *
         *     Admite tres modos:
         *
         *     - `text` + `template`: un archivo CSV, XLSX, XLS, TXT o JSON con encabezados canónicos en español o en inglés.
         *     - `text` + `free`: un TXT o CSV sin estructura de plantilla.
         *     - `images`: una o varias imágenes PNG, JPG o WEBP en `file[]`, o un archivo .zip de imágenes.
         *
         *     El trabajo nace con `status="pending"` y continúa en segundo plano; el avance se obtiene haciendo un sondeo recurrente a `GET /v1/validations/imports/{id}`.
         *
         *     {% callout type="warning" %}
         *     Solo puede haber un trabajo abierto a la vez: una segunda subida mientras la anterior sigue viva responde con un estado HTTP `422` (con `import_already_in_flight` en el cuerpo).
         *
         ...
         *
         *     {% callout type="info" %}
         *     Esta operación solo admite autenticación por cookie de sesión. Una clave de API responde con un estado HTTP `401`.
         *
         *     {% /callout %}
         */
        post: operations["createValidationImport"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
    ...
    "/validations/imports/template": {
        ...
        /**
         * Descargar la plantilla de importación de validaciones
         * @description Descarga una plantilla con los encabezados canónicos y tres filas de ejemplo, en el idioma de la sesión.
         *
         *     La importación acepta encabezados en español o en inglés.
         *
         *     {% callout type="info" %}
         *     Esta operación solo admite autenticación por cookie de sesión. Una clave de API responde con un estado HTTP `401`.
         *
         *     {% /callout %}
         */
        get: operations["downloadValidationImportTemplate"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
    ...
    "/validations/imports/{id}": {
        ...
        /**
         * Consultar el estado de un trabajo de importación
         * @description Devuelve el estado y los contadores de un trabajo de importación de validaciones.
         *
         *     Lo que informa **depende de la fase**:
         *
         *     - Antes de confirmar la importación: Devuelve las filas válidas, corregibles, fatales y duplicadas.
         *     - Durante la ejecución de la importación: las filas iniciadas y sus resultados.
         *
         *     {% callout type="info" %}
         *     Esta operación solo admite autenticación por cookie de sesión. Una clave de API responde con un estado HTTP `401`.
         *
         *     {% /callout %}
         */
        get: operations["getValidationImport"];
        put?: never;
        post?: never;
        /**
         * Cancelar un trabajo de importación
         * @description Cancela un trabajo de importación y marca como `skipped` las filas que seguían pendientes de procesamiento (`queued`), de modo que al llegar al procesador son omitidas y bloquea su persistencia.
         *
         *     Las validaciones que ya habían comenzado pueden terminar, y **la cuota ya consumida no se restituye**.
         *
         *     {% callout type="info" %}
         *     Esta operación solo admite autenticación por cookie de sesión. Una clave de API responde con un estado HTTP `401`.
         *
         *     {% /callout %}
         */
        delete: operations["cancelValidationImport"];
        options?: never;
        head?: never;
        patch?: never;
        trace?: never;
    };
    "/validations/imports/{id}/preview": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        /**
         * Listar las filas leídas del trabajo de importación
         * @description Devuelve las filas extraídas del archivo a importar, con los datos interpretados, su clasificación y los errores detectados.
         *
         *     `buckets[]` limita la respuesta a una o varias clasificaciones. La respuesta no está paginada.
         *
         *     {% callout type="info" %}
         *     Esta operación solo admite autenticación por cookie de sesión. Una clave de API responde con un estado HTTP `401`.
         *
         *     {% /callout %}
         */
        get: operations["previewValidationImport"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
    ...
    "/validations/imports/{id}/rows/{row_id}": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        post?: never;
        /**
         * Eliminar una fila del trabajo de importación
         * @description Elimina una fila de un trabajo de importación y vuelve a calcular los contadores de la vista previa.
         *
         *     Solo se permite mientras el trabajo está en `preview_ready`. Una vez
         *     confirmado, la operación responde con estado HTTP `422` y
         *     `job_not_editable`.
         *
         *     {% callout type="info" %}
         *     Esta operación solo admite autenticación por cookie de sesión. Una clave de API responde con un estado HTTP `401`.
         ...
         *     {% /callout %}
         */
        delete: operations["deleteValidationImportRow"];
        options?: never;
        head?: never;
        /**
         * Editar una fila del trabajo de importación
         * @description Actualiza uno o varios campos interpretados de una fila extraida de un trabajo de importación y vuelve a calcular su clasificación.
         *
         *     El trabajo debe estar en `preview_ready`.
         *
         *     {% callout type="info" %}
         *     Esta operación solo admite autenticación por cookie de sesión. Una clave de API responde con un estado HTTP `401`.
         *
         *     {% /callout %}
         */
        patch: operations["patchValidationImportRow"];
        trace?: never;
    };
    "/validations/imports/{id}/commit": {
        parameters: {
            query?: never;
            header?: never;
            path?: never;
        ...
        put?: never;
        /**
         * Confirmar e iniciar el trabajo de importación
         * @description Confirma un trabajo con estado `preview_ready` e inicia en segundo plano la validación de cada fila (que representa cada transferencia).
         *
         *     Si esas filas no caben en la cuota disponible, responde con un estado HTTP `422` (con `quota_insufficient` en el cuerpo) sin iniciar ninguna. Si el lote excede el tope de carga masiva del plan (`bulk_validations_max`, por lote), responde `422` con `plan_cap_exceeded`, sin iniciar tampoco ninguna fila.
         *
         *     El cuerpo es opcional y permite fijar los canales de la notificación `validation_import.completed`.
         *
         *     {% callout type="info" %}
         *     Las importaciones masivas no admiten reintentos automáticos. Si llega `retry_policy`, se ignora y la respuesta incluye la cabecera `X-Retry-Policy-Ignored: bulk_imports_not_supported`.
         *
         *     {% /callout %}
         *
         *     {% callout type="info" %}
         ...
         *     {% /callout %}
         */
        post: operations["commitValidationImport"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
    ...
    "/webhooks": {
         ...
         *     Ese secreto verifica cada entrega: el HMAC-SHA256 del cuerpo recibido debe coincidir con la cabecera `X-{Brand}-Signature: sha256=<hex>`.\
         *     \
    ...
    "/plans": {
         ...
         * @description Devuelve los planes de suscripción activos, cada uno con sus viñetas
         *     (`features[]`) ya filtradas a las activas.
         *
         *     La landing pública usa el endpoint paralelo
         *     [`GET /v1/plans/public`](/es/v1/plans/list-public-plans), sin
         *     autenticación y con los precios embebidos.
         *
         *     La suscripción vigente del usuario (`plan_slug`, `effective_limit` y
         *     fechas del ciclo) se consulta en
         *     [`GET /v1/billing/subscription`](/es/v1/billing/billing-get-subscription).
         */
    ...
    "/plans/{slug}/prices": {
        ...
        /**
         * Listar los precios de un plan
         * @description Devuelve los precios activos del plan identificado por `slug`, desglosados
         *     por moneda, intervalo y tipo.
         *
         *     El `stripe_price_id` no se expone: el servidor resuelve el precio en
         *     `POST /v1/billing/checkout-session`; el cliente sólo envía `plan_slug`,
         *     `currency` y `billing_interval`.
         *
         *     `meta.plan` lleva un mini-resumen del plan (`billing_model`, `trial_days`,
         *     `included_validations`) para que la interfaz muestre el contexto sin otra
         *     consulta.
         *
         *     {% callout type="info" %}
         *     Si el plan no existe, está desactivado o tiene `is_public = 0`, responde con
         *     un estado HTTP `404` (con `plan_not_found` en el cuerpo), sin revelar qué
         *     planes existen.
         *
         *     {% /callout %}
         */
    ...
    "/playground/templates": {
        ...
        /**
         * Listar las plantillas de petición del banco de pruebas
         * @description Devuelve el catálogo de plantillas pre-cargadas para {% concept slug="playground" %}el Playground{% /concept %} del
         *     portal de documentación, filtrado por los permisos del usuario autenticado.
         *     Solo aparecen las operaciones para las que el llamador tiene acceso.
         *
         *     Disponible tanto para usuarios regulares como para administradores.
         */
        get: operations["playgroundTemplates"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
    ...
    "/playground/proxy": {
        ...
        put?: never;
        /**
         * Reenviar una petición desde el banco de pruebas sin exponer la clave
         * @description Reenvía, del lado del servidor, una petición del Playground del portal de
         *     documentación al endpoint `/v1` destino. El servidor descifra la clave de API del
         *     propio usuario y la usa para firmar la llamada al destino, de modo que la
         *     clave nunca llega al navegador y no hace falta revelarla.
         *
         *     Autenticado por cookie de sesión (más el token CSRF en el header de las
         *     mutaciones). El destino debe ser una operación documentada y el usuario debe
         *     cumplir los permisos que esa operación exige; en caso contrario se devuelve
         *     `403`.
         *
         *     En `mode: live` la respuesta es la del endpoint destino sin modificar: el
         *     código de estado y el cuerpo son los que produjo él. En `mode: playground`
         *     la llamada se simula tras pasar los mismos filtros de permiso y visibilidad,
         *     de modo que sirve para probar la forma de una petición sin producir efectos
         *     ni consumir cuota.
         */
        post: operations["playgroundProxy"];
        delete?: never;
        options?: never;
        head?: never;
        patch?: never;
    ...
    "/account-directory/lookup": {
        ...
        /**
         * Buscar banco en catálogo global por últimos dígitos
         * @description Busca en el catálogo global de cuentas la entrada más reciente cuyo sufijo
         *     coincida con los dígitos indicados. Útil como alternativa cuando la búsqueda
         *     en la lista propia del usuario (`/v1/beneficiaries/lookup`) no devuelve
         *     resultado.
         *
         *     El campo `account_number` se **omite intencionalmente** de la respuesta
         *     para evitar la enumeración del catálogo. La respuesta contiene
         *     únicamente los metadatos del banco necesarios para identificar al
         *     receptor. Cuando múltiples entradas coinciden con el mismo sufijo, se
         *     devuelve la más reciente (por `last_seen_at DESC`).
         *
         *     Límite de tasa: 60 peticiones por minuto por usuario (no por IP).
         */
        get: operations["lookupAccountDirectory"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
    ...
    schemas: {
        ...
        UserMeBundle: {
            ...
            api_key?: {
                 ...
                 * @example example_a1b2
                 */
                prefix?: string;
                /**
                 * @description Últimos 4 caracteres de la clave de API, con los que se pinta enmascarada (`example_a1b2••••••f9c2`). `null` en las cuentas anteriores a ese campo, hasta que se rellena.
                 * @example f9c2
                 */
                last_4?: string | null;
            } | null;
        ...
        /** @description Una sesión de la cuenta. Nace al entrar, al superar la autenticación de 2 factores (2FA) o al cambiar la contraseña, y de cada una se guarda desde dónde se abrió y hasta cuándo vale. */
        UserSession: {
            /**
             * Format: uuid
             * @description Identificador único de la sesión (UUID v7, ordenado por tiempo). Es el valor que consume `DELETE /v1/users/me/sessions/{id}`.
             * @example 01918b3c-7e21-7c5f-9a44-2e5fa1b9c001
             */
            id: string;
            /**
             * @description `true` cuando esta sesión es la que está haciendo la petición.
             * @example true
             */
            is_current: boolean;
            /**
             * @description Dirección IP desde la que se abrió la sesión. `null` cuando no llegó al servidor en ese momento.
             * @example 203.0.113.42
             */
            ip: string | null;
            /**
             * @description Cabecera `User-Agent` capturada al abrir la sesión, que es de donde sale el «navegador · sistema» que muestra la interfaz. `null` cuando el cliente no la envió.
             * @example Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0
             */
            user_agent: string | null;
            /**
             * Format: date-time
             * @description Fecha y hora en que se abrió la sesión, en ISO 8601 UTC. Coincide con el inicio de sesión, la 2FA o el cambio de contraseña que la creó.
             * @example 2026-05-28T10:00:00Z
             */
            created_at: string;
            /**
             * Format: date-time
             * @description Fecha y hora de la última petición autenticada que usó esta sesión, en ISO 8601 UTC. Se actualiza en cada petición y es lo que ordena la lista.
             * @example 2026-05-28T11:30:00Z
             */
            last_used_at: string;
            /**
             * Format: date-time
             * @description Fecha y hora en que la sesión caduca sola, en ISO 8601 UTC. Son 4 horas desde que se abrió, o 24 cuando al entrar se pidió recordar el dispositivo. Pasado ese instante la sesión se rechaza con `session_expired`.
             * @example 2026-05-29T10:00:00Z
             */
            expires_at: string;
        };
        /** @description Las sesiones activas del usuario autenticado, con su cantidad. Las revocadas y las caducadas quedan fuera, y la lista viene ordenada por uso reciente. */
        UserSessionsList: {
            /** @description Sesiones activas en la cuenta de usuario (ordenadas por `last_used_at` de más reciente a más antigua). Una por cada inicio de sesión que sigue vivo. */
            sessions: components["schemas"]["UserSession"][];
            /**
             * @description Cantidad de sesiones activas en la cuenta de usuario (conteo de `sessions`).
             * @example 3
             */
            count: number;
        };
        ...
        /** @description Recurso JSON:API con el resultado estructural de la cuenta validada. */
        ValidateAccountResource: {
            /**
             * @description Tipo del recurso JSON:API. Siempre `account_validation`.
             * @example account_validation
             * @enum {string}
             */
            type: "account_validation";
            /** @description Datos del resultado de la validación estructural. */
            attributes: {
                /**
                 * @description Texto recibido (sin normalizar).
                 * @example 012180004412345678
                 */
                input: string;
                /**
                 * @description Longitud en dígitos del input normalizado.
                 * @example 18
                 */
                length: number;
                /**
                 * @description `true` cuando el input original contenía únicamente dígitos (sin separadores).
                 * @example true
                 */
                is_numeric: boolean;
                /**
                 * @description Tipo de cuenta detectado por longitud, o `unknown` para entradas fuera de rango. Los tipos reconocidos son `clabe`, `card` y `phone`.
                 * @example clabe
                 * @enum {string}
                 */
                account_type: "clabe" | "card" | "phone" | "unknown";
                /**
                 * @description `true` cuando el input está completo para su tipo (18 dígitos en CLABE, 16 en tarjeta, 10 en celular).
                 * @example true
                 */
                is_complete: boolean;
                /**
                 * @description `true` cuando el dígito de control de la CLABE o el Luhn de la tarjeta es válido. Para celular siempre `false` (no aplica).
                 * @example true
                 */
                checksum_valid: boolean;
                /**
                 * @description Dígito de control CLABE calculado a partir de los primeros 17 dígitos. `null` cuando el tipo no es CLABE o el cálculo no es aplicable.
                 * @example 8
                 */
                computed_control_digit?: string | null;
                /**
                 * @description CLABE de 18 dígitos auto-completada cuando el input venía con 17 dígitos y el control pudo calcularse. `null` en los demás casos.
                 * @example 012180004412345678
                 */
                auto_completed?: string | null;
                /** @description Metadatos del banco resuelto, o `null` cuando no se pudo identificar. Se deriva del prefijo o BIN. */
                bank?: {
                    /**
                     * @description Código SPEI (5 dígitos) (cuando el banco está mapeado).
                     * @example 40012
                     */
                    banxico_code?: string | null;
                    /**
                     * @description Nombre oficial del banco resuelto.
                     * @example BBVA MEXICO
                     */
                    name?: string;
                } | null;
                /** @description Metadatos adicionales del BIN cuando `account_type=card` y hubo coincidencia en el directorio. `null` en cualquier otro caso. */
                card?: {
                    /**
                     * @description Prefijo BIN, los 6 primeros dígitos de la tarjeta, con el que se buscó el banco emisor.
                     * @example 411111
                     */
                    bin?: string;
                    /**
                    ...
                    brand?: string | null;
                    /**
                     * @description Tipo de tarjeta (`debit`, `credit`, etc.).
                     * @example debit
                     */
                    type?: string | null;
                    /**
                     * @description Nivel de la tarjeta (`classic`, `gold`, etc.).
                     * @example classic
                     */
                    level?: string | null;
                    /**
                     * @description ISO 3166-1 alpha-2 del país emisor.
                     * @example MX
                     */
        ...
        /** @description Entrada de una importación masiva en modo `text`, con un archivo CSV, XLSX, XLS, TXT o JSON. */
        ValidationImportTextInput: {
            /**
             * @description Modalidad de entrada. Siempre `text`. (enum property replaced by openapi-typescript)
             * @enum {string}
             */
            upload_type: "text";
            /**
             * Format: binary
             * @description Archivo con encabezados canónicos en modo `template` o contenido libre en modo `free`. Tamaño máximo: 50 MB.
             * @example transferencias-2026-04.csv
             */
            file: string;
            /**
             * @description Modo de lectura del archivo — `template`: Respeta los encabezados canónicos; `free`: Formato libre, el sistema deduce la estructura.
        ...
        /** @description Entrada de una importación masiva en modo `images`, con una o varias imágenes PNG, JPG o WEBP. */
        ValidationImportImagesInput: {
            /**
             * @description Modalidad de entrada. Siempre `images`. (enum property replaced by openapi-typescript)
             * @enum {string}
             */
            upload_type: "images";
            /** @description Imágenes que integran el trabajo de importación. Cada una admite hasta 12 MB (la carga completa hasta 50 MB). */
            "file[]": string[];
        };
        /** @description Entrada de una importación masiva cuyo `upload_type` selecciona texto o imágenes. */
        CreateValidationImportRequest: components["schemas"]["ValidationImportTextInput"] | components["schemas"]["ValidationImportImagesInput"];
        /** @description Estado de un trabajo de importación recién creado. */
        CreateValidationImportAttributes: {
            /**
             * @description Estado inicial del trabajo de importación. Siempre `pending`.
             * @example pending
             * @enum {string}
             */
            status?: "pending";
        ...
        /**
         * Format: binary
         * @description Plantilla de importación en JSON entregada como archivo descargable.
         */
        DownloadValidationImportTemplateResponse: string;
        /** @description Recurso JSON:API de un trabajo de importación masiva y su estado de ejecución. */
        ValidationImportJob: components["schemas"]["JsonApiResourceBase"] & {
            /**
             * @description Tipo de recurso JSON:API. Siempre `validation_import`.
             * @example validation_import
             * @enum {string}
             */
            type?: "validation_import";
            /**
             * @description Identificador numérico del trabajo de importación, expresado como cadena.
             * @example 42
             */
            id?: string;
            /** @description Datos del trabajo de importación. */
            attributes?: {
                /**
                 * @description Estado actual — `pending`: Recibido y pendiente de lectura; `parsing`: Extrayendo y validando filas; `preview_ready`: Lectura terminada y pendiente de revisión; `committing`: Procesando las filas aprobadas; `completed`: Todas las filas procesadas; `failed`: Error irrecuperable; `cancelled`: Trabajo cancelado por el usuario.
                 * @example preview_ready
                 * @enum {string}
                 */
                status?: "pending" | "parsing" | "preview_ready" | "committing" | "completed" | "failed" | "cancelled";
                /**
                 * @description Modalidad de entrada: `text` para datos estructurados o libres; `images` para imágenes de comprobantes.
                 * @example text
                 * @enum {string}
                 */
                upload_type?: "text" | "images";
                /**
                 * @description Formato del archivo subido (`csv`, `xls`, `xlsx`, `txt`, `json`, `zip_text` o `zip_images`).
                 * @example csv
                 * @enum {string}
                 */
                file_format?: "csv" | "xls" | "xlsx" | "txt" | "json" | "zip_text" | "zip_images";
                /**
                 * @description Modo de lectura del archivo — `template`: Respeta los encabezados canónicos; `free`: Formato libre, el sistema deduce la estructura.
                 * @example template
                 * @enum {string}
                ...
                parse_mode?: "template" | "free";
                /**
                 * @description Cantidad de filas extraídas del archivo.
                 * @example 120
                 */
                total_rows?: number;
                /** @description Tope de carga masiva del plan (`bulk_validations_max`, por lote) comparado contra `total_rows`, para avisar en la vista previa antes de que la confirmación rechace el lote de verdad con `422` `plan_cap_exceeded`. */
                plan_cap?: {
                    /**
                     * @description Tope de filas por lote del plan activo.
                     * @example 100
                     */
                    max?: number;
                    /** @description `true` cuando `total_rows` ya excede `max`. */
                    exceeded?: boolean;
                };
                /**
                 * @description Cantidad de filas sin errores, listas para la confirmación.
                ...
                valid_count?: number;
                /**
                 * @description Cantidad de filas con errores que admiten corrección.
                 * @example 10
                 */
                correctable_count?: number;
                /**
                 * @description Cantidad de filas que no pueden procesarse con sus datos actuales.
                 * @example 3
                 */
                fatal_count?: number;
                /**
                 * @description Cantidad de filas detectadas como duplicadas dentro del trabajo.
                 * @example 2
                 */
                duplicate_count?: number;
                /**
                 * @description Cantidad de filas enviadas a validación tras confirmar el trabajo.
                 * @example 115
                 */
                dispatched_count?: number;
                /**
                 * @description Cantidad de filas cuya validación terminó en `valid`.
                 * @example 100
                 */
                completed_valid_count?: number;
                /**
                 * @description Cantidad de filas cuya validación terminó en `not_found`.
                 * @example 8
                 */
                completed_not_found_count?: number;
                /**
                 * @description Cantidad de filas cuya validación terminó en `invalid`.
                 * @example 3
                 */
                completed_invalid_count?: number;
                /**
                 * @description Cantidad de filas cuya validación terminó en `cep_unavailable`.
                 * @example 2
                 */
                completed_cep_unavailable_count?: number;
                /**
                 * @description Cantidad de filas cuya validación terminó en `returned` (la operación se liquidó y después se devolvió).
                 * @example 0
                 */
                completed_returned_count?: number;
                /**
                 * @description Cantidad de filas cuya validación terminó en `error` o `failed`.
                 * @example 2
                 */
                completed_error_count?: number;
                /**
                 * @description Cantidad de filas omitidas durante la ejecución del trabajo.
                 * @example 0
                 */
                skipped_count?: number;
                /**
                 * @description `true` cuando se utilizó OCR durante la lectura. Solo es relevante cuando `upload_type` es `images`.
                 * @example false
                 */
                ocr_invoked?: boolean;
                /**
                 * @description Tokens de entrada acumulados por el OCR del trabajo. Es `0` si `ocr_invoked` es `false`.
                 * @example 0
                 */
                ocr_input_tokens?: number;
                /**
                 * @description Tokens de salida acumulados por el OCR del trabajo. Es `0` si `ocr_invoked` es `false`.
                 * @example 0
                 */
                ocr_output_tokens?: number;
                /**
                 * @description Código estable del fallo del trabajo. `null` cuando no terminó en `failed`.
                 * @example null
                 */
                error_code?: string | null;
                /**
                 * @description Resumen legible del fallo del trabajo. `null` cuando no terminó en `failed`.
                 * @example null
                 */
                error_summary?: string | null;
                created_at?: components["schemas"]["TimestampUTC"];
        ...
        /** @description Fila interpretada de un trabajo de importación, desde la lectura del archivo hasta el resultado de la validación. */
        ValidationImportRow: components["schemas"]["JsonApiResourceBase"] & {
            /**
             * @description Tipo de recurso JSON:API. Siempre `validation_import_row`.
             * @example validation_import_row
             * @enum {string}
             */
            type?: "validation_import_row";
            /**
             * @description Identificador numérico de la fila, expresado como cadena.
             * @example 1
             */
            id?: string;
            /** @description Datos de la fila interpretada. */
            attributes?: {
                /**
                 * @description Posición de la fila dentro del archivo original, empezando en `0`.
                 * @example 0
                 */
                row_index?: number;
                /**
                 * @description Estado en el ciclo de vida de la fila — `pending_ocr`: fila esperando extracción OCR; `valid`: fila lista para confirmación; `correctable`: fila con errores pero admite correcciones; `fatal`: fila con errores, no procesable con sus datos actuales; `duplicate`: fila duplicada dentro del trabajo de importación; `queued`: fila iniciando procesamiento tras confirmación; `processing`: fila en procesamiento activo; `validated_valid` / `validated_not_found` / `validated_invalid` / `validated_cep_unavailable` / `validated_returned` / `validated_error`: resultado final de la validación de la fila (`validated_returned`: la operación se liquidó y después se devolvió); `skipped`: fila omitida; `cancelled`: fila cancelada por el usuario.
                 * @example valid
                 * @enum {string}
                 */
                status?: "pending_ocr" | "valid" | "correctable" | "fatal" | "duplicate" | "queued" | "processing" | "validated_valid" | "validated_not_found" | "validated_invalid" | "validated_cep_unavailable" | "validated_returned" | "validated_error" | "skipped" | "cancelled";
                /**
                 * @description Clave de rastreo extraída del archivo. `null` cuando no se pudo extraer.
                 * @example HSBC211231ABC123456
                 */
                parsed_clave_rastreo?: string | null;
                /**
                 * @description Referencia numérica extraída del archivo. `null` cuando no se pudo extraer.
                 * @example 1234567
                 */
                parsed_referencia_numerica?: string | null;
                /**
                 * Format: date
                 * @description Fecha extraída e interpretada del archivo (en formato `YYYY-MM-DD`); `null` cuando no se pudo extraer.
                 * @example 2026-05-15
                 */
                parsed_fecha?: string | null;
                /**
                 * Format: double
                 * @description Monto de la transacción extraída del archivo (en pesos mexicanos — MXN). `null` cuando no se pudo extraer.
                 * @example 1500
                 */
                parsed_monto?: number | null;
                /**
                 * @description Cuenta receptora extraída del archivo: CLABE (18 dígitos), tarjeta (16 dígitos) o celular/DiMo (10 dígitos). `null` cuando no se pudo extraer.
                 * @example 032180000118359719
                 */
                parsed_cuenta_beneficiaria?: string | null;
                /**
                 * @description Código SPEI (4 o 5 dígitos), extraído del archivo para el banco emisor. `null` cuando no está disponible.
                 * @example 40012
                 */
                parsed_emisor?: string | null;
                /**
                 * @description Código SPEI (4 o 5 dígitos), extraído del archivo para el banco receptor. `null` cuando no está disponible.
                 * @example 40021
                 */
                parsed_receptor?: string | null;
                /**
                 * @description Códigos de error detectados durante la lectura. `null` cuando la fila no tiene errores.
                 * @example [
                 *       "fecha_missing"
                 *     ]
                 */
                ...
                /**
                 * @description Avisos de cambios aplicados durante la normalización; `null` cuando no hubo cambios.
                 * @example null
                 */
                corrections_applied?: string[] | null;
                /**
                 * @description Campos corregidos durante la vista previa mediante `PATCH /v1/validations/imports/{id}/rows/{row_id}`. Estos valores tienen precedencia durante la confirmación. `null` cuando no hay correcciones.
                 * @example null
                 */
                user_overrides?: Record<string, never> | null;
            };
        };
        /** @description Cuerpo de `PATCH /v1/validations/imports/{id}/rows/{row_id}`. */
        PatchValidationImportRowRequest: {
            /** @description Recurso JSON:API de la petición. */
            data?: {
                /**
                 * @description Tipo de recurso JSON:API. Siempre `validation_import_row`.
                 * @example validation_import_row
                 ...
                 */
                type?: "validation_import_row";
                /** @description Correcciones de los campos interpretados. Debe contener al menos una propiedad admitida; las demás se ignoran. */
                attributes?: {
                    /**
                     * @description Valor corregido de `clave_rastreo` para esta fila.
                     * @example MXBA20250315001234
                     */
                    parsed_clave_rastreo?: string;
                    /**
                     * @description Valor corregido de `referencia_numerica` para esta fila.
                     * @example 1234567
                     */
                    parsed_referencia_numerica?: string;
                    /**
                     * @description Fecha corregida (en formato `YYYY-MM-DD` o en el formato original) para esta fila.
                     * @example 2026-04-30
                     */
                    parsed_fecha?: string;
                    /**
                     * @description Cadena con el monto corregido para esta fila. Se limita a 255 caracteres antes de procesarse.
                     * @example 15000.50
                     */
                    parsed_monto?: string;
                    /**
                     * @description Cuenta beneficiaria corregida (CLABE, tarjeta o teléfono) para esta fila.
                     * @example 646180157000000004
                     */
                    parsed_cuenta_beneficiaria?: string;
                    /**
                     * @description Banco emisor corregido para esta fila.
                     * @example BBVA MEXICO
                     */
                    parsed_emisor?: string;
                    /**
                     * @description Banco receptor corregido para esta fila.
                     * @example STP
                     */
                    parsed_receptor?: string;
                };
            };
        };
        /** @description Cuerpo opcional de `POST /v1/validations/imports/{id}/commit`. */
        CommitValidationImportRequest: {
            /** @description Recurso JSON:API con las preferencias de la confirmación. */
            data?: {
                /**
                 * @description Tipo de recurso JSON:API. Siempre `validation_import_commit`.
                 * @example validation_import_commit
                 * @enum {string}
                 */
                type?: "validation_import_commit";
                /** @description Preferencias aplicadas al confirmar el trabajo de importación. */
                attributes?: {
                    /** @description Canales de notificación para el evento `validation_import.completed`. Admite `email`, `telegram`, `push` e `in_app`. Si se omite, se usan los canales predeterminados de la cuenta. */
                    forced_notification_channels?: ("email" | "telegram" | "push" | "in_app")[];
                };
            };
        };
        /** @description Estado inicial de la ejecución de un trabajo de importación confirmado. */
        CommitValidationImportAttributes: {
            /**
             * @description Estado del trabajo de importación tras aceptar la confirmación. Siempre `committing`.
             * @example committing
             * @enum {string}
             */
            status?: "committing";
            /**
             * @description Cantidad de filas enviadas a validación.
             * @example 120
             */
            dispatched_count?: number;
            /**
             * @description Cantidad de filas incluidas en el trabajo confirmado.
             * @example 128
             */
            total_rows?: number;
        };
        ...
        WebhookEndpoint: components["schemas"]["JsonApiResourceBase"] & {
            ...
            attributes: {
                ...
                /** @description Fecha y hora del último intento de entrega, en ISO 8601 UTC. `null` cuando el endpoint aún no recibió ninguna entrega. */
                last_delivery_at?: components["schemas"]["TimestampUTC"] | null;
                /**
                 * @description Clave compartida para verificar firmas. **Solo presente** en la respuesta de creación y de rotación del endpoint; se omite en cualquier otra respuesta.
        ...
        UpdateWebhookRequest: {
            /**
             * Format: uri
             * @description Nueva URL receptora del webhook (endpoint). Es rechazada si no lleva protocolo seguro — HTTPS.
             * @example https://example.com/webhooks/entregas
             */
            url?: string;
            ...
            events?: ("validation.completed" | "validation.failed" | "validation.error" | "validation.retry.scheduled" | "validation.retry.resolved" | "validation.retry.exhausted" | "billing.payment_succeeded" | "billing.payment_failed" | "billing.trial_will_end" | "billing.subscription_canceled" | "billing.invoice_upcoming")[];
            /**
             * @description Nueva etiqueta libre para distinguir el endpoint webhook de los demás. `null` la elimina.
             * @example Alta de pagos en el ERP
             */
            description?: string | null;
            /**
        ...
        SendWebhookTestAttributes: {
            ...
            /**
             * @description Detalle de la falla cuando `delivered=false`. `null` cuando la entrega fue exitosa.
        ...
        WebhookDelivery: components["schemas"]["JsonApiResourceBase"] & {
            ...
            attributes: {
                ...
                attempt?: number;
                /**
                 * @description Estado actual de la entrega — `pending`: Aún sin entregar; `retrying`: Reintentando; `success`: Entregada; `failed`: Intentos agotados.
                 * @example pending
                 * @enum {string}
                 */
                status?: "pending" | "retrying" | "success" | "failed";
                /** @description Fecha y hora del próximo reintento programado, en ISO 8601 UTC. `null` cuando la entrega es terminal (entregada o agotada). */
                next_retry_at?: components["schemas"]["TimestampUTC"] | null;
                /**
                 * @description Mensaje de error del intento de entrega. `null` cuando la entrega llegó al receptor.
                 * @example Connection timed out after 10s
                 */
                error_message?: string | null;
                created_at?: components["schemas"]["TimestampUTC"];
        ...
        /** @description Atributos de un plan de suscripción: límites de uso, modelo de facturación, visibilidad y orden dentro del catálogo. */
        Plan: {
            /**
             * @description Identificador único y legible del plan. Es inmutable y se usa en otras peticiones, como `PUT /v1/users/me/subscription`.
             * @example free
             */
            slug: string;
            /**
            ...
            /**
             * @description `true` cuando el plan aparece en el catálogo público y puede contratarse por autoservicio; `false` cuando queda oculto de ambos.
             * @example true
             */
            is_public: boolean;
            /**
             * @description Identificador del Stripe Product asociado. `null` cuando el plan no está conectado a Stripe.
             * @example prod_NfX1Y2Z3W4V5U6
             */
            stripe_product_id: string | null;
            /**
             * @description `true` cuando el plan está activo y puede asignarse. Los catálogos para usuarios excluyen los planes inactivos; administración los conserva.
             * @example true
             */
            is_active: boolean;
            /**
             * @description Posición del plan dentro del catálogo. Los valores menores aparecen primero.
             * @example 1
             */
            sort_order: number;
            /**
             * @description Cantidad de cuentas no revocadas con una suscripción activa a este plan.
             * @example 47
             */
            user_count: number;
            /**
            ...
            /**
             * @description Tope de reintentos por validación para el plan. `null` cuando hereda el valor global predeterminado.
             * @example 5
             */
            retry_max_retries: number | null;
            /**
             * @description Tope de reintentos de validación pendientes por usuario para el plan. `null` cuando hereda el valor global predeterminado.
             * @example 50
             */
            retry_max_pending_per_user: number | null;
            /**
             * @description Tope de reintentos de validación despachados por día y usuario para el plan. `null` cuando hereda el valor global predeterminado.
             * @example 200
             */
            retry_max_dispatched_per_day: number | null;
            /**
             * @description Tope de comprobantes por lote de carga masiva. No admite `-1`: "sin tope" se representa con un número grande.
             * @example 100
             */
            bulk_validations_max: number;
            /**
             * @description Tope mensual de SMS para la autenticación de 2 factores (2FA) en login, por ciclo de suscripción. **No** cubre la verificación de teléfono. Al agotarse, el login cae a correo electrónico automáticamente sin avisar al usuario. El login biométrico nunca lo consulta — siempre reta por correo electrónico.
             * @example 60
             */
            sms_2fa_monthly_cap: number;
            /** @description Características bilingües del plan, ordenadas por `sort_order`. Los catálogos para usuarios sólo incluyen las activas. */
            features?: components["schemas"]["PlanFeature"][];
        };
        /** @description Un plan activo del catálogo autenticado, con sus características bilingües activas. */
        ListPlansItem: components["schemas"]["JsonApiResourceBase"] & {
            /**
             ...
             */
            id: string;
            attributes: components["schemas"]["Plan"] & Record<string, never>;
        };
        ...
        /** @description Recurso JSON:API de un precio activo del plan. */
        PlanPriceResource: components["schemas"]["JsonApiResourceBase"] & {
            /**
             * @description Tipo de recurso JSON:API. Siempre `plan_price`.
             * @example plan_price
             * @enum {string}
             */
            type: "plan_price";
            /**
             * @description Identificador compuesto `<currency>_<interval>_<kind>` (ej. `MXN_month_base`).
             * @example MXN_month_base
             */
            id: string;
            attributes: components["schemas"]["PlanPriceAttributes"];
        };
        /** @description Respuesta de `GET /v1/plans/{slug}/prices` con los precios activos y un resumen del plan en `meta.plan`. */
        ListPlanPricesResponse: components["schemas"]["SuccessEnvelope"] & {
            /** @description Lista de precios activos del plan. */
            data: components["schemas"]["PlanPriceResource"][];
            /** @description Metadatos complementarios — incluye `plan` además de los campos estándar del envelope. */
            meta: {
                /** @description Mini-resumen del plan referenciado. */
                plan: {
                    /**
                     * @description Identificador estable del plan en URLs y referencias.
                     * @example pro
                     */
                    slug: string;
                    /**
        ...
        /** @description Envoltorio JSON:API con las categorías y plantillas disponibles para el playground, filtradas por los permisos del usuario que llama. */
        PlaygroundTemplatesResponse: {
            ...
            data: {
                /**
                 * @description Tipo del recurso, fijo para esta operación. Forma parte de la identidad del recurso en la envoltura JSON:API. Siempre `playground_templates`.
                 * @example playground_templates
                 * @enum {string}
                 */
                type: "playground_templates";
                /** @description Atributos del recurso: los datos propiamente dichos, separados de su identidad. */
                attributes: {
                    /** @description Categorías ordenadas según manifest sidebar-order.json (W2): primero las de `groups.reference`, luego `groups.admin`, luego el resto alfabéticamente. Sin manifest: primero las no-admin (preservando el orden YAML), luego las `Admin: *`. */
                    categories: {
                        /**
                         * @description Tag de OpenAPI (e.g. `Validations`, `Admin: Users`).
                         * @example Validations
                         */
                        id: string;
                        /**
                         * @description Mismo valor que `id`: La aplicación lo usa como encabezado de la categoría.
                         * @example Validations
                         */
                        label: string;
                        /**
                         * @description Para categorías admin, el sub-heading de grupo (e.g. "Soporte", "Operaciones") del manifest sidebar-order.json. `null` para categorías no-admin y para categorías admin que no inician un nuevo grupo.
                         * @example Soporte
                         */
                        bucket_label?: string | null;
                        /** @description Plantillas que quien llama puede ejecutar. La lista ya viene filtrada por sus permisos. */
                        templates: {
                            /**
                             * @description OpenAPI `operationId` (e.g. `createValidation`). Fallback: `<method>_<path-with-slashes-replaced>`.
                             * @example createValidation
                             */
                            id: string;
                            /**
                             * @description `summary` localizada de la operación (cae al `id` si no hay summary).
                             * @example Validar una transferencia
                             */
                            label: string;
                            /**
                             * @description `description` localizada de la operación. El playground UI lo muestra arriba del URL bar.
                             * @example Valida una transferencia SPEI contra el CEP de Banxico.
                             */
                            description?: string;
                            /**
                             * @description Método HTTP con el que se ejecuta la plantilla. Uno de `GET`, `POST`, `PUT`, `PATCH` o `DELETE`.
                             * @example GET
                             * @enum {string}
                             */
                            method: "GET" | "POST" | "PUT" | "DELETE" | "PATCH";
                            /**
                             * @description URL con prefijo `/v1` ya aplicado (e.g. `/v1/validate`).
                             * @example /v1/validate
                             */
                            url: string;
                            /** @description Pre-llenado del playground HTTP Client. Origen: `requestBody.content.application/json.example`. `null` cuando la operación no declara example. */
                            default_body?: Record<string, never> | null;
                            /**
                             * @description Eje de visibilidad del modelo de audiencia de 2 ejes (`x-visibility`): `public` (visible para cualquier llamador) o `admin` (solo admin/owner). El UI del playground lo usa para etiquetar la plantilla. Por defecto `public` si la operación no declara `x-visibility`.
                             * @example public
                             * @enum {string}
                             */
                            visibility?: "public" | "admin";
                            /**
                             * @description Cómo se autentica la operación: `none` (sin credenciales), `api_key` (Authorization: Bearer example_…) o `cookie` (sesión de navegador). El UI del playground lo usa para etiquetar la plantilla. Por defecto `none` si la operación no declara `x-auth` (coincide con el default del filtro de audiencia).
                             * @example none
                             * @enum {string}
                             */
                            auth?: "none" | "api_key" | "cookie";
                            /**
                             * @description Permiso que quien llama tiene, con la forma `resource:action` que usan todos —así, en inglés, es como se guardan—. Se comprueba antes de incluir la plantilla, así que lo que se lista es lo que esa persona puede ejecutar.
                             * @example validations:create
                             */
                            permission: string;
                            /**
                             * @description `true` cuando el endpoint tiene al menos un escenario de simulación disponible (`scenarios` no vacío). El UI lo usa para mostrar/ocultar el modo "playground" (simulación sin efectos reales) junto al modo "live".
                             * @example true
                             */
                            supports_playground?: boolean;
                            /** @description Escenarios de simulación que el playground ofrece para este endpoint. Para las validaciones (`/v1/validate`, `/v1/validate-ocr`) son los desenlaces de Banxico (`valid` / `not_found` / `cep_unavailable` / `returned` / `error`) más, en OCR, un escenario `ocr_extraction` con `inputs` editables. Para el resto de endpoints, un escenario por cada status documentado en `responses` (200/201/422/403/404/409…), con su ejemplo de respuesta. El UI lista estos escenarios para que el usuario elija antes de ejecutar en modo playground. */
                            scenarios?: {
                                /**
                                 * @description Identificador del escenario. Validaciones: `valid` / `not_found` / `cep_unavailable` / `returned` / `error` / `ocr_extraction`. Resto: el código de status documentado (e.g. `200`, `422`).
                                 * @example not_found
                                 */
                                id: string;
                                /**
                                 * @description Etiqueta legible para el selector de escenario.
                                 * @example Comprobante inexistente
                                 */
                                label: string;
                                /**
                                 * @description Descripción del escenario (puede estar vacía).
                                 * @example Banxico no encuentra la operación.
                                 */
                                description?: string;
                                /**
                                 * @description Código HTTP que devolverá el endpoint real bajo este escenario. Las validaciones siempre son 200 (el veredicto vive en el body); el resto refleja el status documentado.
                                 * @example 404
                                 */
                                status: number;
                                /**
                                 * @description Agrupador semántico para el badge del UI: `banxico` / `ocr` (validaciones) o `success` / `client_error` / `server_error` / `other` (resto, derivado del rango del status).
                                 * @example banxico
                                 */
                                group: string;
                                /** @description Campos editables pre-poblados que alimentan el escenario (solo presente cuando aplica, e.g. la extracción OCR de `ocr_extraction`). El UI los muestra como formulario editable; los valores viajan de vuelta en `scenario.inputs` al ejecutar. */
                                inputs?: Record<string, never>;
                            }[];
                            /** @description Lista plana de parámetros de entrada resueltos desde `operation.parameters` (path/query/header) y `requestBody.content.application/json.schema` (body). Cada entrada incluye nombre, ubicación (`in`), tipo, flag `required`, descripción y restricciones opcionales (enum, pattern, min/max, etc.). Campos null se omiten del JSON. */
                            params?: {
                                /**
                                 * @description Nombre del parámetro o campo del body.
                                 * @example clave_rastreo
                                 */
                                name: string;
                                /**
                                 * @description Ubicación del parámetro: `path`, `query`, `header` (de `operation.parameters`) o `body` (de `requestBody.schema`).
                                 * @example path
                                 * @enum {string}
                                 */
                                in: "path" | "query" | "header" | "body";
                                /**
                                 * @description Tipo OpenAPI primitivo (`string`, `number`, `integer`, `boolean`, `array`, `object`). Por defecto `string` si falta en el spec.
                                 * @example string
                                 */
                                type: string;
                                /**
                                 * @description Formato opcional (`date`, `date-time`, `uuid`, `email`, `double`, etc.).
                                 * @example date
                                 */
                                format?: string;
                                /**
                                 * @description `true` cuando el parámetro es obligatorio según el spec. Por defecto, `false`.
                                 * @example true
                                 */
                                required: boolean;
                                /**
                                 * @description Descripción del parámetro (del spec). Vacío si no declarada.
                                 * @example Clave de rastreo de la transferencia.
                                 */
                                description?: string;
                                /** @description Valor de ejemplo del parámetro, con la forma que este acepta. */
                                example?: unknown;
                                /** @description Valores permitidos. Presente solo cuando el spec declara `enum`. */
                                enum?: unknown[];
                                /**
                                 * @description Expresión regular de validación. Presente solo cuando el spec declara `pattern`.
                                 * @example ^[A-Za-z0-9]{1,30}$
                                 */
                                pattern?: string;
                                /**
                                 * @description Valor mínimo numérico. Incluye `exclusiveMinimum` cuando es numérico (OpenAPI 3.0 style).
                                 * @example 0
                                 */
                                minimum?: number;
                                /**
                                 * @description Valor máximo numérico. Incluye `exclusiveMaximum` cuando es numérico.
                                 * @example 100
                                 */
                                maximum?: number;
                                /**
                                 * @description Longitud mínima de string (`minLength`).
                                 * @example 1
                                 */
                                min_length?: number;
                                /**
                                 * @description Longitud máxima de string (`maxLength`).
                                 * @example 30
                                 */
                                max_length?: number;
                                /** @description Para `type=array`, el schema reducido del elemento (`type`, `format` resueltos). */
                                items?: Record<string, never>;
                                /** @description Para `type=object` en el primer nivel de anidamiento, lista recursiva de descriptores de los sub-campos (mismo shape). Objetos más profundos se emiten sin `properties`. */
                                properties?: Record<string, never>[];
                            }[];
                        }[];
                    }[];
                    /**
                     * @description Conteo total de plantillas que el caller tiene permitido ver (suma de `templates.length` a través de todas las `categories`).
                     * @example 42
                     */
                    total: number;
                };
        ...
        /** @description Envoltorio JSON:API de la petición del proxy del Playground. El servidor reenvía la operación descrita en `data.attributes` al endpoint `/v1` destino usando la clave de API del propio usuario (descifrada del lado servidor), de modo que la clave nunca llega al navegador. */
        PlaygroundProxyRequest: {
            /** @description Cuerpo de la respuesta. Todo el contenido cuelga de aquí, según la envoltura JSON:API. */
            data: {
                /**
                 * @description Tipo del recurso JSON:API. Siempre `playground_request`.
                 * @example playground_request
                 * @enum {string}
                 */
                ...
                attributes: {
                    /**
                     * @description `live` reenvía la petición real al endpoint destino y devuelve su respuesta tal cual (passthrough). `playground` ejecuta el motor de simulación con escenarios y devuelve la misma respuesta que daría el endpoint real para el escenario elegido, sin efectos colaterales (sin cuota, webhooks ni notificaciones).
                     * @example live
                     * @enum {string}
                     */
                    mode: "live" | "playground";
                    /**
                     * @description Método HTTP de la operación destino. Uno de `GET`, `POST`, `PUT`, `PATCH` o `DELETE`.
                     * @example GET
                     * @enum {string}
                     */
                    method: "GET" | "POST" | "PUT" | "DELETE" | "PATCH";
                    /**
                     * @description Ruta absoluta del endpoint destino, con el prefijo `/v1` (por ejemplo `/v1/beneficiaries`). Debe corresponder a una operación documentada; rutas no listadas devuelven `403 path_not_allowed`.
                     * @example /v1/beneficiaries
                     */
                    path: string;
                    /** @description Parámetros de query a agregar a la URL destino. Opcional. */
                    query?: {
                        [key: string]: unknown;
                    };
                    /** @description Cabeceras del cliente a reenviar. Solo se propaga una lista acotada (p. ej. `Idempotency-Key`, `Accept-Language`); el resto se descarta. La credencial del destino la pone el servidor — nunca se reenvía `Authorization` ni `Cookie` del cliente. */
                    headers?: {
                        [key: string]: string;
                    };
                    /** @description Cuerpo de la petición destino para métodos distintos de `GET`. Opcional / `null` cuando no aplica. */
                    body?: {
                        [key: string]: unknown;
                    } | null;
                    /** @description Escenario de simulación para `mode: playground` (ignorado en `mode: live`). `id` selecciona el escenario (p. ej. `valid`, `not_found`) de los que lista `GET /v1/playground/templates`. */
                    scenario?: {
                        /**
                         * @description Identificador del escenario pre-cargado.
                         * @example valid
                         */
                        id?: string;
                        /** @description Valores con los que se rellena el escenario antes de reenviarlo. Las claves son libres: dependen del escenario que nombre `scenario_id`. */
                        inputs?: {
                            [key: string]: unknown;
                        };
                    };
        ...
        /** @description Recurso JSON:API con la entrada del directorio que coincide con el sufijo consultado. */
        AccountDirectoryLookupResource: {
            /**
             * @description Tipo del recurso JSON:API. Siempre `account_directory_lookup`.
             * @example account_directory_lookup
             * @enum {string}
             */
            type: "account_directory_lookup";
            /** @description Atributos canónicos de la entrada del directorio (banco resuelto y última observación). */
            attributes: {
                /**
                 * @description Tipo de cuenta inferido para la entrada del catálogo — `clabe`: CLABE de 18 dígitos; `card`: Tarjeta de 16; `phone`: Teléfono de 10.
                 * @example clabe
                 * @enum {string}
                 */
                account_type: "clabe" | "card" | "phone";
                /**
                 * @description Código Banxico SPEI de 5 dígitos. `null` cuando la entrada no tiene banco resuelto (p. ej. BIN desconocido).
                 * @example 40012
                 */
                bank_code: string | null;
                /**
                 * @description Nombre del banco asociado a la entrada del catálogo.
                 * @example BBVA MEXICO
                 */
                bank_name: string;
                /**
                 * Format: date-time
                 * @description Timestamp ISO 8601 UTC de la última observación de la cuenta.
                 * @example 2026-05-01T14:30:00Z
                 */
                last_seen_at: string;
            };
        };
        /** @description Respuesta de `GET /v1/account-directory/lookup?last_digits=`. Entrada más reciente del catálogo global de cuentas cuyo sufijo coincide con los dígitos enviados. **No incluye `account_number`** — solo el banco y la fecha de última observación, para evitar enumeración del catálogo. */
        LookupAccountDirectoryResponse: components["schemas"]["SuccessEnvelope"] & {
            data: components["schemas"]["AccountDirectoryLookupResource"];
        };
    };
    responses: {
        ...
        /** @description Este endpoint requiere autenticación por cookie (sesión interactiva). Las claves de API Bearer son rechazadas. */
        AuthMethodDenied: {
            headers: {
                [name: string]: unknown;
            };
...
export interface operations {
        ...
        responses: {
            ...
            200: {
                ...
                content: {
                     ...
                     *           "sessions": [
                     *             {
                     *               "id": "01918b3c-7e21-7c5f-9a44-2e5fa1b9c001",
                     *               "is_current": true,
                     *               "ip": "203.0.113.42",
                     ...
                     *             },
                     *             {
                     *               "id": "01918a91-2c44-7100-8b22-9a4b1c7d0010",
                     *               "is_current": false,
                     *               "ip": "198.51.100.7",
                     *               "user_agent": "Mozilla/5.0 (iPhone; CPU iPhone OS 17_4) Safari",
                     *               "created_at": "2026-05-27T18:15:00Z",
                     *               "last_used_at": "2026-05-28T08:02:00Z",
                     *               "expires_at": "2026-05-28T18:15:00Z"
                     *             }
                     *           ],
                     *           "count": 2
                     *         }
                    ...
                    "application/json": components["schemas"]["SuccessEnvelope"] & {
                        data?: {
                            ...
                            type?: string;
                            attributes?: components["schemas"]["UserSessionsList"];
                        };
                    };
                };
            };
            401: components["responses"]["Unauthorized"];
            403: components["responses"]["AuthMethodDenied"];
        };
    };
    revokeMySessions: {
        parameters: {
            query?: {
                /**
                 * @description Qué sesiones conservar. **El único valor admitido es** `current`, que
                 *     deja viva la sesión desde la que se hizo la petición; cualquier otro
                 *     responde con un estado HTTP `422`.
                 *
                 *     Sin este parámetro se cierran **todas** las sesiones web (incluida la
                 *     actual) — lo que toca cuando alguien más ha podido entrar, y lo que no
                 *     toca cuando solo se quiere echar a otro dispositivo.
                 */
                except?: "current";
            };
            header?: never;
        ...
        requestBody?: never;
        responses: {
            /** @description Sesiones revocadas. La respuesta incluye `revoked_count` y `mode`. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                     ...
                     *       },
                     *       "meta": {
                     *         "request_id": "req_01HV8K9D9Q4Z"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["SuccessEnvelope"] & {
                        data?: {
                            /** @example sessions_revoked */
                            type?: string;
                            attributes?: {
                                /**
                                 * @description Cantidad de sesiones revocadas en esta llamada.
                                 * @example 3
                                 */
                                revoked_count: number;
                                /**
                                 * @description `all_except_current` cuando se pasó `?except=current`; `all` cuando no se especificó.
                                 * @example all_except_current
                                 * @enum {string}
                                 */
                                mode: "all" | "all_except_current";
                            };
                        };
                    };
                };
            ...
            401: components["responses"]["Unauthorized"];
            403: components["responses"]["AuthMethodDenied"];
            /** @description Valor inválido para `?except` (solo se acepta `current`). */
            422: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    revokeMySession: {
        parameters: {
            ...
            header?: never;
            path: {
                /** @description Identificador único (UUID) de la sesión web a revocar. */
                id: string;
            };
            cookie?: never;
        };
        requestBody?: never;
        responses: {
            /** @description Sesión revocada. La respuesta devuelve el `session_id` revocado. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                     ...
                     *         "attributes": {
                     *           "session_id": "01918a91-2c44-7100-8b22-9a4b1c7d0010"
                     *         }
                     *       },
                     *       "meta": {
                     *         "request_id": "req_01HV8K9DAB12"
                     *       }
                     *     }
                     */
                    "application/json": components["schemas"]["SuccessEnvelope"] & {
                        data?: {
                            ...
                            attributes?: {
                                /**
                                 * Format: uuid
                                 * @description UUID de la sesión que se acaba de revocar.
                                 * @example 01918a91-2c44-7100-8b22-9a4b1c7d0010
                                 */
                                session_id: string;
                            };
                        };
            ...
            401: components["responses"]["Unauthorized"];
            403: components["responses"]["AuthMethodDenied"];
            /** @description Sesión no encontrada, no pertenece al usuario autenticado, o ya había sido revocada previamente. Las tres condiciones devuelven el mismo código `session_not_found_or_already_revoked` para evitar enumeración. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            /** @description El `id` enviado no es un UUID válido. Código: `invalid_session_id`. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    validateAccount: {
        parameters: {
            query: {
                /**
                 * @description Dígitos de la cuenta (completos o parciales). CLABE, tarjeta o celular. Mínimo 1 dígito, máximo 19.
                 * @example 012180004412345678
                 */
                account: string;
                /**
                 * @description Fuerza la interpretación del input en vez de auto-detectar por longitud. `clabe` resuelve el banco por prefijo aunque la CLABE esté incompleta (sin confundirla con tarjeta); `bin` busca un BIN de 6–8 dígitos en el directorio. Omitir para auto-detección.
                 * @example clabe
                 */
                type?: "clabe" | "bin";
            };
        ...
        requestBody?: never;
        responses: {
            /** @description Resultado de la validación estructural. El cuerpo siempre tiene forma uniforme; los flags `checksum_valid` y `account_type` comunican el éxito o el motivo del rechazo estructural. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ValidateAccountResponse"];
                };
            };
            /** @description `query_param_account_required`: El parámetro `account` viene vacío. */
            400: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    createValidationImport: {
        ...
        requestBody: {
            content: {
                 ...
                 *       "upload_type": "text",
                 *       "parse_mode": "template",
                 *       "file": "@transferencias.csv"
                 *     }
                 */
                "multipart/form-data": components["schemas"]["CreateValidationImportRequest"];
            };
        };
        responses: {
            /** @description Trabajo de importación creado en estado `pending`. La lectura comienza de forma asíncrona. */
            201: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    ...
                    "application/json": components["schemas"]["SuccessEnvelope"] & {
                        data?: components["schemas"]["JsonApiResourceBase"] & {
                            ...
                            type?: "validation_import";
                            /**
                             * @description ID numérico de la importación como cadena (convención JSON:API).
                             * @example 42
                             */
                            id?: string;
                            attributes?: components["schemas"]["CreateValidationImportAttributes"];
                        };
                    };
                };
            };
            /** @description Requiere una sesión autenticada; las claves de API no se admiten. */
            401: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            /** @description El archivo no pasó la comprobación previa o ya hay un trabajo en curso. Los códigos son `file_required` cuando no llega archivo, `import_already_in_flight` cuando la cuenta de usuario ya tiene un trabajo abierto, `multi_upload_not_allowed_for_text` al proporcionar varios archivos en modo texto, `file_too_large`, `unsupported_format`, `persist_failed`, `zip_pack_failed` y `upload_error`. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            /** @description No fue posible crear el trabajo (`import_failed`). El archivo debe enviarse de nuevo. */
            500: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    downloadValidationImportTemplate: {
        parameters: {
            query: {
                /**
                 * @description Formato de la plantilla que se descarga. Valores aceptados: `csv`, `xlsx`, `xls`, `txt`, `json`.
                 * @example csv
                 */
                format: "csv" | "xlsx" | "xls" | "txt" | "json";
            };
        ...
        requestBody?: never;
        responses: {
            /** @description Archivo plantilla generado. El `Content-Type` corresponde al formato solicitado. */
            200: {
                headers: {
                    /** @description Nombre canónico de descarga de la plantilla. */
                    "Content-Disposition"?: string;
                    [name: string]: unknown;
                };
                content: {
                    ...
                    "text/plain": string;
                    "application/json": components["schemas"]["DownloadValidationImportTemplateResponse"];
                };
            };
            /** @description Requiere una sesión autenticada; las claves de API no se admiten. */
            401: {
            ...
            /** @description Formato fuera de la lista admitida (código `invalid_format`). */
            422: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    getValidationImport: {
        parameters: {
            ...
            path: {
                /**
                 * @description Identificador numérico del trabajo de importación.
                 * @example 42
                 */
                id: number;
            };
        ...
        requestBody?: never;
        responses: {
            /** @description Estado del trabajo de importación con los contadores de vista previa y ejecución. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["SuccessEnvelope"] & {
                        data?: components["schemas"]["ValidationImportJob"];
                    };
                };
            };
            /** @description Requiere una sesión autenticada; las claves de API no se admiten. */
            ...
            /** @description Trabajo no encontrado o ajeno a la cuenta de usuario (`not_found`). */
            404: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    cancelValidationImport: {
        parameters: {
            query?: never;
            header?: never;
            path: {
                /**
                 * @description Identificador numérico de la importación.
                 * @example 42
                 */
                id: number;
            };
        ...
        responses: {
            ...
            /** @description Trabajo no encontrado en la cuenta del usuario autenticado (`not_found`). */
            404: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    previewValidationImport: {
        parameters: {
            query?: {
                /**
                 * @description Filtro — Clasificaciones incluidas: `valid`, `correctable`, `fatal` y `duplicate`. Si se omite, devuelve todas las filas.
                 * @example [
                 *       "correctable",
                 *       "fatal"
                 *     ]
                 */
                buckets?: ("valid" | "correctable" | "fatal" | "duplicate")[];
            };
            header?: never;
            path: {
                /**
        ...
        requestBody?: never;
        responses: {
            /** @description Filas de la vista previa con metadatos de totales y del trabajo. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["SuccessEnvelope"] & {
                        data?: components["schemas"]["ValidationImportRow"][];
                        meta?: {
                            /** @description Cantidad de filas devueltas tras aplicar el filtro `buckets[]`. */
                            total_rows?: number;
                            job?: components["schemas"]["ValidationImportJob"];
                        };
                    };
                };
            };
    ...
    deleteValidationImportRow: {
        parameters: {
            ...
            path: {
                ...
                id: number;
                /**
                 * @description Identificador numérico de la fila a eliminar.
                 * @example 7
                 */
                row_id: number;
            };
        ...
        requestBody?: never;
        responses: {
            /** @description Fila eliminada. Sin cuerpo. */
            204: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            /** @description Trabajo o fila no encontrados (`not_found`). */
            404: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            /** @description El trabajo ya salió de `preview_ready` (`job_not_editable`). */
            422: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    patchValidationImportRow: {
        parameters: {
            ...
            path: {
                ...
                id: number;
                /**
                 * @description Identificador numérico de la fila a editar.
                 * @example 7
                 */
                row_id: number;
            };
        ...
        requestBody: {
            content: {
                "application/json": components["schemas"]["PatchValidationImportRowRequest"];
            };
        };
        responses: {
            /** @description Fila actualizada con su nueva clasificación. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            413: components["responses"]["PayloadTooLarge"];
            /** @description La corrección no se pudo aplicar. Los códigos son `job_not_editable` cuando el trabajo ya salió del estado de revisión, y `no_valid_fields` cuando el cuerpo no contiene ningún campo editable. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    commitValidationImport: {
        ...
        requestBody?: {
            content: {
                "application/json": components["schemas"]["CommitValidationImportRequest"];
            };
        };
        responses: {
            /** @description Trabajo iniciado. El estado de la importación cambia a `committing`. `dispatched_count` indica cuántas filas se iniciaron; `total_rows` refleja el total de la importación. */
            202: {
                headers: {
                    /** @description Solo está presente cuando el cuerpo incluye `retry_policy`. Indica que la política fue aceptada, pero no se aplicará porque las importaciones masivas no admiten reintentos. */
                    "X-Retry-Policy-Ignored"?: "bulk_imports_not_supported";
                    [name: string]: unknown;
                };
                content: {
                    /**
                    ...
                    "application/json": components["schemas"]["SuccessEnvelope"] & {
                        data?: components["schemas"]["JsonApiResourceBase"] & {
                            ...
                            type?: "validation_import";
                            id?: string;
                            attributes?: components["schemas"]["CommitValidationImportAttributes"];
                        };
                    };
                };
            };
            ...
            /** @description La importación no está en un estado válido para confirmación (`job_not_ready`: No es `preview_ready`), la cuota mensual es insuficiente para iniciar todas las filas confirmables (`quota_insufficient`), o el lote excede el tope de carga masiva del plan (`plan_cap_exceeded`). */
            422: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    createWebhook: {
        ...
        responses: {
            /** @description Endpoint webhook creado. El campo `secret` aparece **solo en esta respuesta** y no puede volver a consultarse: es el que verifica la firma `X-{Brand}-Signature: sha256=<hex>` que acompaña a cada entrega. */
            201: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    listAllDeliveries: {
        parameters: {
            query?: {
                ...
                endpoint_id?: string;
                /**
                 * @description Filtro — Desenlace del intento de entrega. `success` llegó. `failed` no llegó y ya no se reintenta. `retrying` falló pero queda algún intento. `pending` todavía no se ha intentado.
                 * @example failed
                 */
    ...
    exportAllDeliveries: {
        parameters: {
            query?: {
                ...
                /**
                 * @description \
                 *     Filtro — Desenlace del intento de entrega. `success` llegó. `failed` no llegó y ya no se reintenta. `retrying` falló pero queda algún intento. `pending` todavía no se ha intentado.
                 * @example failed
                 */
                status?: "success" | "failed" | "retrying" | "pending";
                /**
    ...
    listPlans: {
        ...
        requestBody?: never;
        responses: {
            /** @description Listado de planes activos. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["SuccessEnvelope"] & {
                        data: components["schemas"]["ListPlansItem"][];
                    };
                };
            };
            401: components["responses"]["Unauthorized"];
    ...
    listPlanPrices: {
        ...
        requestBody?: never;
        responses: {
            /** @description Precios activos del plan + resumen en `meta.plan`. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["ListPlanPricesResponse"];
                };
            };
            401: components["responses"]["Unauthorized"];
            /** @description `plan_not_found`: El slug no existe, el plan está desactivado, o `is_public = 0`. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            /** @description `plan_slug_invalid`: El `slug` no coincide con `[a-z0-9_-]{1,50}`. */
            422: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    playgroundTemplates: {
        parameters: {
            query?: {
                /**
                 * @description Acota las plantillas a las que se ejecutan con clave de API. **El único valor admitido es `api_key`**; sin el parámetro se devuelven todas las que quien llama puede ejecutar.
                 * @example api_key
                 */
                mode?: "api_key";
            };
        ...
        requestBody?: never;
        responses: {
            /** @description Catálogo de plantillas agrupado por tag de OpenAPI. Cada plantilla incluye método, URL, body por defecto, visibility, auth y permiso requerido. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            429: {
                headers: {
                    /** @description Segundos a esperar antes de reintentar. Coincide con la ventana de rate-limit del endpoint. */
                    "Retry-After"?: number;
                    [name: string]: unknown;
                };
                content: {
            ...
            /** @description La especificación de la API no se pudo leer. Los códigos son `openapi_spec_missing`, `openapi_spec_invalid_yaml` y `openapi_spec_parse_failed`; los tres apuntan al mismo problema del lado del servidor y no dependen de la petición. */
            500: {
                headers: {
                    [name: string]: unknown;
                };
    ...
    playgroundProxy: {
        ...
        requestBody: {
            content: {
                "application/json": components["schemas"]["PlaygroundProxyRequest"];
            };
        };
        responses: {
            /** @description Passthrough transparente del endpoint destino en `mode: live`. El código de estado y el cuerpo son los que devolvió el destino (varían según el endpoint y pueden ser cualquier 2xx/4xx documentado por esa operación); este 200 representa el caso de éxito típico. Consulta el endpoint destino para el esquema exacto del cuerpo. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            401: components["responses"]["Unauthorized"];
            /** @description El destino no puede invocarse desde el Playground. `path_not_allowed`: la ruta/método no corresponde a una operación documentada, o es una operación admin para un llamador no-admin. `forbidden`: el llamador no cumple algún permiso que la operación destino exige. */
            403: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            /** @description El usuario no tiene una clave de API almacenada para firmar el reenvío. Hay que generar una clave de API antes de usar el Playground en `mode: live`. */
            409: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                     ...
                     *           "detail": "Esta cuenta no tiene ninguna clave de API. Genera una antes de usar el banco de pruebas en vivo."
                     *         }
                     *       ],
                     *       "meta": {
                     *         "version": "1.50.8",
            ...
            413: components["responses"]["PayloadTooLarge"];
            /** @description El cuerpo de la petición es inválido: `data.attributes` ausente, `mode` no soportado, `method` inválido o `path` mal formado (debe ser una ruta absoluta que empiece con `/v1`). */
            422: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                     ...
                     *           "detail": "Método ausente o no válido. Se espera uno de: GET, POST, PUT, DELETE, PATCH."
                     *         }
                     *       ],
                     *       "meta": {
                     *         "version": "1.50.8",
            ...
            429: components["responses"]["RateLimited"];
            /** @description Falló la simulación (`simulation_failed`), o la especificación de la API no se pudo leer (`openapi_spec_missing`, `openapi_spec_invalid_yaml`, `openapi_spec_parse_failed`). Ninguno depende de la petición. */
            500: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            /** @description El reenvío al endpoint destino no pudo completarse (tiempo de espera, conexión rechazada o error de transporte). No se filtran detalles internos del fallo. */
            502: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                     ...
                     *           "detail": "No se pudo completar la petición al servicio de destino."
                     *         }
                     *       ],
                     *       "meta": {
                     *         "version": "1.50.8",
    ...
    lookupAccountDirectory: {
        parameters: {
            query: {
                /**
                 * @description Últimos 4–10 dígitos de la cuenta a buscar. Menos de 4 dígitos (ambigüedad excesiva) o más de 10 → `400 invalid_last_digits_length`.
                 * @example 1234
                 */
                last_digits: string;
            };
        ...
        requestBody?: never;
        responses: {
            /** @description Entrada más reciente del catálogo que coincide con los dígitos indicados. */
            200: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                    "application/json": components["schemas"]["LookupAccountDirectoryResponse"];
                };
            };
            /** @description Parámetro `last_digits` ausente (`query_param_last_digits_required`) o fuera del rango 4–10 dígitos (`invalid_last_digits_length`). */
            400: {
                headers: {
                    [name: string]: unknown;
                };
            ...
            401: components["responses"]["Unauthorized"];
            403: components["responses"]["Forbidden"];
            /** @description Ninguna entrada del catálogo coincide con los dígitos indicados. */
            404: {
                headers: {
                    [name: string]: unknown;
                };
                content: {
                     ...
                     *         {
                     *           "status": "404",
                     *           "code": "account_not_in_catalog",
                     *           "detail": "No encontramos ninguna entrada en el directorio de cuentas para esos dígitos."
                     *         }
                     *       ],
                     *       "meta": {
                     *         "version": "1.47.0",
